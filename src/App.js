...
const App = () => {
  ...
  // Track current preview/object URLs to revoke on cleanup
  ...
  // Compute actual image dimensions when preview changes
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
      if (!preview) {
      ...
      try {
        ...
        if (!cancelled) {
          setActualDims({ w: img.naturalWidth, h: img.naturalHeight });
        }
      } catch {
  ...
  // JPEG binary search to meet target size
  const encodeJpegToTarget = async (canvas, targetBytes) => {
    let lo = 0.5;
    let hi = 0.95;
    let best = null;

    // Try a small number of iterations for speed
    for (let i = 0; i < 8; i++) {
      const q = (lo + hi) / 2;
    ...
    // Fallback if still above target
    if (!best) {
  ...
  // PNG encode (note: no quality param; size target not guaranteed)
  const encodePng = async (canvas) => {
    const blob = await canvasToBlob(canvas, "image/png");
    return blob;
  };

  // PDF export using jsPDF: embeds the canvas as an image
  const encodePdf = async (canvas) => {
    if (!jsPDFLib) {
      throw new Error(
        "PDF generation library not available. Install 'jspdf' to enable PDF export."
      );
    }
    // Define PDF page size in points (1pt = 1/72 inch). Use 1px ≈ 1pt for simplicity.
    ...
    // Prefer JPEG inside PDF for smaller size; fall back to PNG
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      // Draw to canvas; change fit to "contain" or "cover" if desired
      const fitMode = "stretch";
      const canvas = drawToCanvas(imgEl, targetW, targetH, fitMode);
      ...
      if (fileType === "jpeg") {
        ...
        if (outBlob && outBlob.size > targetBytes) {
          setWarning(
            "Target size could not be reached without excessive quality loss. Provided the closest possible file."
          );
        }
        ...
        if (outBlob && outBlob.size > targetBytes) {
          setWarning(
            "PNG is lossless; exact max size not guaranteed. Consider JPEG/PDF if strict size is required."
          );
        }
        ...
        try {
          ...
          if (outBlob && outBlob.size > targetBytes) {
            setWarning(
              "PDF sizes vary; exact max size not guaranteed. Consider reducing dimensions or using JPEG."
            );
          }
        } catch (e) {
          // Fallback: export PNG if PDF lib missing
          setWarning(
            "PDF export not available (jspdf not installed). Exported PNG instead."
          );
          outBlob = await encodePng(canvas);
          outMime = "image/png";
          outExt = "png";
      ...
      setCompressedFile(new File([outBlob], getDownloadFileName(outName, outExt), { type: outMime }));
      setPreview(outUrl);
    ...
    <div className="app-root">
      <header className="app-header">
        <div className="container">
          <h1 className="app-title">Image Resizer & Compressor</h1>
          <p className="app-subtitle">Resize, compress, and export as JPEG/PNG/PDF—fast and private, all in your browser.</p>
        </div>
      </header>
        ...
        <section className="card upload-card">
          <label className="file-input-label" htmlFor="file-input">
            <div className="file-drop">
              <div className="file-icon" aria-hidden>🗂️</div>
          ...
          <input id="file-input" className="file-input" type="file" accept="image/*" onChange={handleUpload} />
        </section>
          ...
          <>
            <section className="card controls-card">
              ...
              <div className="actions">
                ...
                <button className="btn btn-ghost" onClick={handleDownload} disabled={!compressedFile || isProcessing}>
                  Download
                </button>
              </div>
              ...
              <div className="preview-header">
                ...
                {compressedFile && fileType !== "pdf" && actualDims && (
                  <div className="meta">
                    ...
                    <span className="meta-chip">Dimensions: {actualDims.w}×{actualDims.h}</span>
                  </div>
                )}
                {compressedFile && fileType === "pdf" && (
                  <div className="meta">
                    ...
                    <span className="meta-chip">
                      Page: {Math.max(1, Math.floor(width))}×{Math.max(1, Math.floor(height))} pt
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="preview-pane">
                {preview ? (
                  fileType === "pdf" && pdfText ? (
                    <div className="pdf-note">PDF generated. Use Download to save the file.</div>
                  ) : (
                    <img
                      src={preview}
                      alt="Preview"
      ...
      <footer className="app-footer">
        <div className="container footer-grid">
          ...
          <div className="foot-actions">
            <button className="btn btn-plain" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>
              Back to top
            </button>
          </div>
        </div>
